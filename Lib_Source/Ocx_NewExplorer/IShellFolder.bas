...
'Private Function IID_IShellFolder() As GUID
'  Static iid As GUID
...
Public Function GetIShellFolder(isfParent As IShellFolder, pidlRel As Long) As IShellFolder
  ...
  Static iid As GUID
  If (iid.Data1 = 0) Then Call DEFINE_OLEGUID(iid, &H214E6, 0, 0)
  Call isfParent.BindToObject(pidlRel, 0, iid, isf)
  ...
  If nItems Then
    ...
    For i = 1 To nItems '- 1
      ...
      If pidlRel Then
        ...
        Static iid As GUID
        If (iid.Data1 = 0) Then Call DEFINE_OLEGUID(iid, &H214E6, 0, 0)
        If SUCCEEDED(isfParent.BindToObject(pidlRel, 0, iid, isfParent)) = False Then
          Exit For
        End If
